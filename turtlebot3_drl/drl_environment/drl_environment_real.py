...
class DRLEnvironment(Node):
    ...
    *******************************************************************************"""
    ...
    #     self.obstacle_distance *= REAL_LIDAR_DISTANCE_CAP
    def scan_callback(self, msg):
        # convert incoming scan to numpy array
        ranges = np.array(msg.ranges, dtype=np.float32)

        # ranges = np.array(msg.ranges, dtype=np.float32)

        # replace NaNs and inf with max distance
        ranges[np.isnan(ranges)] = REAL_LIDAR_DISTANCE_CAP
        ranges[ranges == float('inf')] = REAL_LIDAR_DISTANCE_CAP



        # apply correction
        ranges = np.clip(ranges - REAL_LIDAR_CORRECTION, 0.0, REAL_LIDAR_DISTANCE_CAP)

        # --- RESAMPLE 223 -> 40 ---
        if len(ranges) != REAL_N_SCAN_SAMPLES:
            ranges = np.interp(
                np.linspace(0, len(ranges) - 1, REAL_N_SCAN_SAMPLES),
                np.arange(len(ranges)),
                ranges
            )

        # normalize to [0,1]
        ranges = ranges / REAL_LIDAR_DISTANCE_CAP

        self.scan_ranges = ranges.tolist()
        self.obstacle_distance = min(ranges) * REAL_LIDAR_DISTANCE_CAP
    ...
    def step_comm_callback(self, request, response):
        ...
        try:
            ...
            # Replace NaNs with 0 to prevent robot freeze
            action_linear = np.nan_to_num(action_linear, nan=0.0)
            action_angular = np.nan_to_num(action_angular, nan=0.0)

            # Optional: clamp to max speeds
            action_linear = np.clip(action_linear, -REAL_SPEED_LINEAR_MAX, REAL_SPEED_LINEAR_MAX)
            action_angular = np.clip(action_angular, -REAL_SPEED_ANGULAR_MAX, REAL_SPEED_ANGULAR_MAX)

        except Exception as e:
            print("Error unnormalizing actions:", e)
            action_linear = 0.0
            action_angular = 0.0
        ...
        if self.local_step % 10 == 0:
            print(f"Rtot: {response.reward:<8.2f}GD: {self.goal_distance:<8.2f}GA: {math.degrees(self.goal_angle):.1f}°\t",
                f"MinD: {self.obstacle_distance:<8.2f}Alin: {request.action[LINEAR]:<7.2f}Aturn: {request.action[ANGULAR]:<7.2f}")
