...
class DRLGazebo(Node):
    ...
    *******************************************************************************"""
    ...
    def generate_goal_pose(self):
        # 1. Define 10 specific test goals (x, y)
        test_goals = [
            [1.5, 0.0], [2.0, -1.5], [-1.0, 2.0], [0.5, 2.2], [-2.0, -2.0],
            [1.8, 1.8], [-1.5, 0.5], [0.0, -2.0], [2.2, -0.5], [-0.5, -1.8]
        ]
        
        # 2. Use the episode counter to pick the goal
        # You will need to define self.episode_idx = 0 in __init__
        if self.episode_idx < len(test_goals):
            self.goal_x = test_goals[self.episode_idx][0]
            self.goal_y = test_goals[self.episode_idx][1]
            self.episode_idx += 1 
        else:
            print("All test goals completed!")
